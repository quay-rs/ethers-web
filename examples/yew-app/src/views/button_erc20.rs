...
async fn transfer(ethereum: &UseEthereum, token_address: H160, to: H160, amount: U256) {
    ...
    {
        if let Ok(tx) = erc20_contract.transfer(to, amount).from(*account).send().await {
        ...
        Callback::from(move |_: MouseEvent| {
            if ethereum.is_connected() {
                ...
                spawn_local(async move {
                    transfer(
                        &eth,
                        "0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512".parse::<H160>().unwrap(),
                        "0xBE565E3eEFcfd58920FfB5048292f67F431356eF".parse::<H160>().unwrap(),
                        U256::from(100000000),
                    )
                    .await;
                });
